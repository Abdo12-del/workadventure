...
## المرحلة 10 — الاختبار الشامل والتوثيق والنشر ✅

### 1) ‏E2E لرحلة الطفل الكاملة (على المحملة الحقيقية)

- ‏`tests/tests/ng_academy_journey.spec.ts` (Playwright، يتبع أعراف المستودع:
  ‏`getPage`/`evaluateScript`/`publicTestMapUrl`) — 3 سيناريوهات:
  1. **الرحلة الكاملة (البند 18)**: دخول → ترحيب جينو (يظهر ويختفي) → الوقوف على
     سجادة التجمع → عرض الصف → «🏫 أين فصلي؟» → اختيار «🔢 فصل الرياضيات» →
     المشي في الصف → التسليم عند الباب (`waitForURL /classroom-math\.wam/`) →
     فقاعة «وصل صفّنا!».
  2. **فصل محفوظ**: طفل يعود لجهازه يرى «🚶 أنضم إلى الصف» مباشرة وينتهي في
     فصل الشطرنج.
  3. **السيادة للطفل**: ضغط سهمين أثناء الصف ⇒ يبقى في `entrance.wam` ولا يُجرّ
     أبدًا (فحص `WA.room.id` — بلا `waitForTimeout`، حسب lint المستودع).
- يتطلب المحملة الكاملة (compose + خرائط NG) مثل بقية الـ75 اختبار E2E — لا
  يُشغَّل داخل صندوق التطوير، ويُشغَّل في CI/المحملة الحقيقية:
  ‏`npm run test --workspace tests -- ng_academy_journey`.

### 2) اختبار التكامل «حياة مدرسية كاملة» (يعمل هنا — أخضر ✅)

- ‏`ng-academy-api/tests/journey.test.ts`: الإدارة تنشئ أمًا وطفلًا وفصل علوم
  بمعلمة → الأم تدخل بالرابط السحري → تفتح جلسة العالم لطفلها (وسم
  ‏`ng-student`) → المعلمة تسجّل حضورًا صامتًا وتُكمل نشاطًا (شارة + نقطة)
  وتكتب ملاحظة → تغذية احتفال الطفل تمتلئ → بوابة ولي الأمر ترى كل شيء →
  وجدران الأمان تُختبر واحدًا واحدًا (الطفل: 403 على الملاحظات/القوائم/الحضور/
  الشارات الذاتية/الإدارة، والغريبة: 403 على طفل غيرها).

### 3) النشر للإنتاج

- ‏`docker-compose.ng-production.yaml` (overlay بنمط المستودع): ‏`ng-postgres`
  بكلمة مرور إلزامية من البيئة، ‏`ng-academy-api` بأمر `npm run start` و
  ‏`NODE_ENV=production` وكل الأسرار إلزامية (فشل سريع `:?`)، ‏`PORTAL_DIST`
  إلى البوابة المبنية (نطاق واحد بلا CORS إضافي)، وخدمة vite التطويرية تُعزل
  خلف profile ‏`dev-portal`. (Helm مؤجَّل — لا مخططات Helm في هذا المستودع
  أصلًا؛ الـoverlay يغطي النشر المطلوب.)

### 4) التوثيق بعلامة NG Academy

- ‏`README.md`: قسم NG Academy في الأعلى — جدول المكوّنات (عالم الطفل/الخرائط/
  الـAPI/البوابة/الهوية)، روابط التوثيق العربي، تشغيل سريع للتطوير والإنتاج.
- ‏`UPGRADE.md`: تحذير «NG fork» قبل أي ترقية من المنبع — قائمة نقاط التماس
  التي يجب دمجها لا استبدالها (العلامة، أمان الطفل، `MediaStore`/المحول الكسول،
  متغيرات البيئة) وتذكير بأن `ngPerfBudget.test.ts` سيكسر أي انتكاسة عمدًا.
- ‏`ng-academy-api/README.md` و`ng-academy-portal/README.md`: محدثان (مسارات
  المرحلتين 7 و8، dev:memory، الإنتاج).

### بوابة قبول المرحلة 10

- ‏`ng-academy-api` — **28/28 ✅** (journey جديد) | typecheck ✅.
- ‏`tests` workspace — eslint ✅ + prettier ✅ لمواصفة الرحلة.
- ‏`ng-academy-portal` — 10/10 ✅ | build ✅. ‏`play` — 938 ✅ (لم تُمسّ هذا الدور).
- ‏prettier ✅ لكل الملفات الجديدة/المعدلة.

---

## الغرف — الدفعة 2: دمج مشاريع تحسين المظهر ✅ (بموافقة المالك)

بعد بحث الدفعة 1، دمجت المشاريع التي تحسّن **مظهر المدرسة** فعليًا، مع الالتزام
بالترخيص والتوثيق:

1. **المصدر**: حزم LimeZu «Modern Interiors» الموزَّعة مع `workadventure/wam-preset-school`
   الرسمي (رخصة «WorkAdventure specific resources» تسمح بالاستخدام والتقصيص داخل خرائط
   WorkAdventure — ومشروعنا خريطة WA مستضافة ذاتيًا). نُسخ الترخيص كاملًا إلى
   ‏`maps/ng-academy/LICENSE.assets` مع إسناد صريح، وأضيفت خاصية `tilesetCopyright`
   لكل ورقة في كل TMJ.
2. **التقصيص (trimming) بدل النسخ الكامل**: بدل 256KB أوراق كاملة (ورقة واحدة 126KB
   كانت تكسر ميزانية ≤100KB/صورة)، استخرجتُ **53 بلاطة مستعملة فقط** في 3 أوراق مصغرة
   ‏(13/25/15 بلاطة) بوزن **1.4–1.9KB للورقة** (palette PNG): ‏`ng-school-classroom.png`,
   ‏`ng-school-generic.png`, ‏`ng-school-musicsport.png`. فهرسة البلاطات تحققت بصريًا
   عبر contact sheets مرقّمة + معاينة رندر TMJ→PNG لكل غرفة + فحص ألفا لكل بلاطة.
3. **ماذا دخل المدرسة**: نوافذ مضيئة ولوحات جدارية وستارة مسرح حمراء في كل الغرف؛
   نباتات ونخل؛ سجاد وملوّنات أرضية؛ رفوف كتب طويلة ومنصة قراءة في المكتبة؛ كرة أرضية
   في فصل العلوم ومختبرها؛ طاولة كمبيوتر في الإنجليزي والتواصل؛ منضدة استقبال زجاجية
   في المدخل؛ وفي قاعة الإنجازات ميداليات وأعمدة كؤوس ومنصة تتويج وواجهة شهادات.
4. **المولّد**: ‏`beginSheets()` يحسب firstgid لكل خريطة حسب الأوراق التي تستعملها فقط
   (ng → classroom → generic → musicsport → Special_Zones بتسلسل متصل تختبره الاختبارات)،
   ودوال ‏`CL/GE/MS` تلتقط من الفهرس المقصوص مع خطأ فوري لأي بلاطة غير مفهرسة.
5. **اختبارات جديدة**: كل ورقة خارجية موثقة (`tilesetCopyright` يحتوي LimeZu) و≤4KB
   و≤32 بلاطة، وكل خريطة تستعمل بلاطاتها فعلاً → 81/81 أخضر + typecheck + eslint.

المستبعد عمدًا (وزن/حاجة): ‏Room_Builder (343KB)، vehicles (631KB)، Celianna (275KB)،
وواجهة WA_UI — تبقى مراجع في جدول بحث الدفعة 1.

## الغرف — الدفعة 1: فصل الحساب ✅

ترقية غرفة غرفة: إعادة تصميم + ميزات تفاعلية لكل طالب.

1. **تصميم جديد للفصل** (‏`generate.mjs`): جدار سبورة رياضيات عريض (2+3=5) + ساعة حائط +
   ملصقان (أشكال وأرقام)، مكتب معلم بتفاحة وكتب، **معداد على كل طاولة تلميذ** (6 طاولات)،
   شريط خط الأعداد (0-8) على الأرضية، و**ركن المعداد** بسجادة ومحطتين ورف (بتصادمات).
   مجموعة البلاطات نمت من 16 إلى 24 (الصف الثالث كله مرسوم بالبكسل داخل المولّد نفسه —
   صفر تبعيات جديدة)، وfirstgid لمناطق Special_Zones انتقل تلقائيًا من 17 إلى 25.
2. **ميزة المعداد لكل طالب** (`NgAbacus.ts` منطق نقي + `NgAbacus.svelte` واجهة):
   4 قضبان (آلاف/مئات/عشرات/آحاد) بخرزات ملوّنة تتحرك بأزرار ＋/ـ، رقم كبير يُقرأ فورًا،
   أهداف عشوائية 1..999 (قابلة للتحقيق دائمًا بثلاثة قضبان)، **حفظ الخرزات محليًا على
   جهاز الطفل** (لا خادم، لا خصوصية مهدورة)، وجينو يحتفل عند كل بناء صحيح — تشجيع
   بلا منافسة. يُفتح تلقائيًا عند دخول منطقة `abacus` ويُغلق عند الخروج منها:
   `NgAreaWatcher` يستمع الآن لـ `onEnterArea` و`onLeaveArea` معًا.
3. **الاختبارات**: ‏`NgAbacus.test.ts` (6 اختبارات: القيمة/الحركة/الأهداف/الحل/الحفظ
   ورفض التخريب) + اختبارا تصميم الفصل في `ngAcademyMaps.test.ts` (ركن المعداد خارج
   منطقة ميكروفونات الدرس livekit + أعداد البلاطات: 8 معدات، 9 سبورة، 14 خط أعداد،
   12 سجادة) → الخرائط + NgAcademy = 79/79 أخضر؛ typecheck + eslint + svelte-check نظيفة.

### بحث GitHub: مشاريع قد تحسّن الغرف (بحث وتوثيق فقط — **بدون دمج** بقرار المالك)

| المشروع                                               | الرابط                                                          | لماذا يهمنا                                                                                                                          |
| ----------------------------------------------------- | --------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ |
| ‏wam-preset-school (رسمي)                             | github.com/workadventure/wam-preset-school                      | خريطة **مدرسة** جاهزة بتنسيق WAM 2.1.0 + بلاطات فصول — مرجع لهيكلة مناطق المدرسة وقواعد ترخيص الأصول (tilesetCopyright/mapCopyright) |
| ‏map-starter-kit + scripting-api-extra                | github.com/workadventure/map-starter-kit                        | سكربتات داخل الخريطة (WA.room/WA.ui/WA.chat) وأدوات جاهزة (بوصلة، شارات أسماء، صوت محيطي) لو أردنا يومًا سكربتات لكل غرفة            |
| ‏workadventure/presentation-map                       | github.com/workadventure/presentation-map                       | خريطة رسمية فيها **سبرايتات معلم وطلاب** — إلهام لمظهر شخصيات الفصول                                                                 |
| ‏DigitaleGesellschaft/workadventure-map-bitwaescherei | github.com/DigitaleGesellschaft/workadventure-map-bitwaescherei | خريطة مجتمعية كبيرة بتنظيم tiles/sounds — أمثلة أصوات محيطة وأثاث                                                                    |
| ‏bizzkoot/aloha                                       | github.com/bizzkoot/aloha                                       | معداد سوروبان PWA بوضع تعليمي + تمارين + لعبة مستويات — أفكار لمراحل متقدمة (خرزة علوية = 5)                                         |
| ‏jccsvq/SorobanTrainer                                | github.com/jccsvq/SorobanTrainer                                | تدريب سوروبان بملفات تمارين (.sbk) — فكرة «تحديات محفوظة» لاحقًا                                                                     |
| ‏Badger0404/sorobanAPP                                | github.com/Badger0404/sorobanAPP                                | تدرّج منهجي جمع→طرح→ضرب مع تتبع تقدم — يُلهم ربط المعداد بشارات الأنشطة                                                              |
| ‏Kenney (أصول CC0)                                    | kenney.nl / github.com/ETdoFresh/kenney.nl                      | بلاطات وأثاث وأصوات **CC0 بلا قيود** — آمنة قانونيًا إذا استبدلنا البلاطات المرسومة يومًا                                            |
| ‏npeguin/workadventure-tutorial                       | github.com/npeguin/workadventure-tutorial                       | بلاطات مدارس RPG (school_b_tileset) + أبواب/سجاد — مرجع بصري                                                                         |

القرار: تبقى هذه المشاريع **مراجع بحثية** — لم يُدمج أي كود أو أصل خارجي (حمايةً لهوية
NG وترخيص الأصول). أي دمج مستقبلي يحتاج موافقة صريحة من المالك + توثيق الرخصة.

## ملخص الخطة — المراحل كلها

| المرحلة | المحتوى                                                                  | الحالة | بوابة القبول                                    |
| ------- | ------------------------------------------------------------------------ | ------ | ----------------------------------------------- |
| 0       | التهيئة وكسر حاجز Protobuf                                               | ✅     | ‏`bootstrap.sh` + خط أساس أخضر                  |
| 1       | الهوية الكاملة (NG Academy، الألوان، i18n)                               | ✅     | لا «WorkAdventure» في واجهة الطفل               |
| 2       | أمان الطفل (دردشة/روابط/إشعارات/كاميرا)                                  | ✅     | اختبارات ChildSafety                            |
| 3       | عالم المدرسة: 13 خريطة مولَّدة                                           | ✅     | 48 اختبار خرائط (WAM 2.1.0 + exits + أمان)      |
| 4       | جينو + المعلمون + رسائل المناطق                                          | ✅     | اختبارات GinoStore (فقاعات/طوابير/مرة-لكل-جلسة) |
| 5       | ‏`ng-academy-api`: Admin API + رابط سحري + أدوار + حضور                  | ✅     | 14 اختبارًا ثم 28                               |
| 6       | رحلة الطفل — بديلها: استكشاف حر + **صف جينو** (حُذف القطار بقرار المالك) | ✅     | ‏NgLine 7 اختبارات + E2E (مرحلة 10)             |
| 7       | الأنشطة والشارات: تشجيع بلا منافسة + احتفال جينو                         | ✅     | 5 اختبارات API + 4 مراقب الإنجازات              |
| 8       | بوابة ولي الأمر + أدوات المعلم + لوحة الإدارة                            | ✅     | 8 اختبارات portal + 10 للبوابة                  |
| 9       | الأداء: MediaPipe كسول، خرائط ‎-73%‎، افتراضيات خفيفة، ميزانيات          | ✅     | ‏ngPerfBudget 5 اختبارات + دليل بناء            |
| 10      | ‏E2E + تكامل الرحلة + إنتاج + توثيق                                      | ✅     | 28/28 + مواصفة Playwright + overlay             |

**خط الأساس النهائي**: ‏play 938 اختبارًا | api 28 | portal 10 | خرائط 48 (ضمن play) |
typecheck+eslint+svelte-check+prettier خضراء في المساحات الثلاث | بناء إنتاجي 15.5ث.

### أعمال مؤجلة بوعي (موثقة، ليست ديونًا صامتة)

- تشغيل E2E الحقيقي يحتاج محملة compose كاملة (خارج صندوق التطوير) — المواصفة جاهزة.
- ‏Helm: لا مخططات في المستودع أصلًا؛ overlay الإنتاج يكفي النشر المطلوب.
- نقل SMTP حقيقي بدل `LogEmailTransport` عند النشر (نقطة واحدة: `server.ts`).
- المرحلة 6 «الكلاسيكية» (شاشة الرحلة/بطاقة الفصل) ألغاها المالك صراحةً وتبقى مؤجلة.
- ربط تسجيل الحضور التلقائي داخل العالم (enter/exit) بمسارات الـAPI — الحضور حاليًا
  يدوي بيد المعلم + صامت عبر `/api/room/access`; يمكن أتمتته لاحقًا من الواجهة.
