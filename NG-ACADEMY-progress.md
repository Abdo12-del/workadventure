...
>
> ⇒ أُلغيت شاشة الرحلة/بطاقة الفصل مؤقتًا؛ بُني بدلها **الاستكشاف الحر + قطار جينو**.
> رحلة الدخول التقليدية وبطاقة الفصل مؤجلتان حتى يطلبهما المالك مجددًا.

## المرحلة 6 (البديلة) — الاستكشاف الحر و«قطار جينو» ✅
...
- **نقطة التجمع**: منطقة WAM جديدة `gathering` على سجادة الاستقبال في المدخل
  (tooltip: «نقطة تجمع قطار جينو 🚂») — أُعيد توليد الخرائط.
- **العرض**: عند الوقوف على السجادة يعرض Gino فقاعة بأزرار (مرة كل 60 ثانية،
  ...
  ومع فصل محفوظ → «🚂 خذني إلى فصلي» / «🚶 سأستكشف بنفسي».
- **الرحلة** `NgTrain.ts`: مشي نقطة-نقطة عبر `GameScene.moveTo` (pathfinding حقيقي)،
  ثم تسليم الطفل عند باب فصله عبر `onMapExit` (نفس مسار بلاطة الباب تمامًا).
  **النزول الفوري**: أي إدخال يدوي (أسهم/WASD/لمس الكانفاس) يوقف القطار صامتًا —
  لا يُجرّ طفل لغرفة ضد إرادته أبدًا.
- فقاعات Gino صارت تدعم **أزرار إجراءات** (`NgBubble.actions` + `ngBubbleAction`).
- المحقن `NgTrainDriver` يجعل آلة الحالة قابلة للاختبار بلا Phaser؛
  `NgTrainSceneAdapter.ts` ربط رفيع بالمشهد الحقيقي.

### بوابة قبول المرحلة 6 البديلة

- `NgTrain.test.ts` — 6 اختبارات ✅: حفظ الفصل ورفض العبث، عرض الـpicker/الرحلة،
  كتم الإلحاح (60ث)، تسلسل نقاط المسار ثم `goRoom` بـ`#from_entrance`، النزول عند
  التدخل اليدوي بلا دخول غرفة، وتغطية أبواب الفصول السبعة.
- امتداد اختبارات الخرائط (48): وجود منطقة `gathering` في المدخل.
- `play` vitest كامل ✅ 131 ملفًا / 928 اختبارًا | typecheck ✅ | eslint ✅ |
  svelte-check 0/0 ✅ | prettier ✅ | `ng-academy-api` 14/14 ✅.

---

## التالي: المرحلة 7 — الأنشطة والشارات والإنجازات

- جداول activities/badges/achievements موجودة (المرحلة 5)؛ يبقى:
  مهام يومية/أسبوعية/مصغّرة تُسلَّم عبر المعلم، ومنح الشارات من بوابة المعلم،
  واحتفال Gino (`ngCelebrate` جاهز) + جدار قاعة الإنجازات.
- تشجيع بلا منافسة: لا لوحات صدارة؛ عرض إنجاز الطفل لنفسه ولولي أمره فقط.
- بوابة القبول: اختبارات ng-academy-api لمنح/سرد الشارات + اختبار احتفال Gino.
